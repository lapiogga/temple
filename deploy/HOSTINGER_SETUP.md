...
### 실패 통보 — healthchecks.io dead-man's switch (2026-07-29 연결 완료)
이 서버엔 MTA 가 없다. **`MAILTO` 를 넣어도 메일은 나가지 않는다** — 그래서 "실패하면 알린다" 대신
**"성공했을 때만 핑을 보내고, 핑이 끊기면 알린다"** 는 뒤집힌 방식을 쓴다.
디스크 풀·인증 만료·cron 중단·서버 다운 어느 쪽이든 핑이 멈추므로 모두 걸린다.

crontab 에 체크별 URL 이 들어 있다(값은 `crontab -l` 참조).
```
HC_URL_DB=...        # backup-db.sh
HC_URL_UPLOADS=...   # backup-uploads.sh
```
healthchecks.io 쪽 설정은 **Period 1 day / Grace 2 hours**. 백업은 19:00·19:05 UTC 에 돈다.

> **체크는 반드시 2개로 분리할 것.** 하나에 둘 다 핑하면 DB 백업이 성공하는 동안
> 이미지 백업이 조용히 죽어도 타이머가 계속 리셋돼 알림이 오지 않는다.

핑은 스크립트 **맨 끝**에 있다. 반출 실패(`exit 1`)든 덤프 실패든 중간에 끝나면 핑에 도달하지 못한다.
검증: `bash -x` 추적으로 정상 경로 curl 핑 1회 / 반출 실패 경로 **0회** 확인(2026-07-29).
